...
class ExpungeOverdueLostTasksActorTest extends AkkaUnitTest with TableDrivenPropertyChecks {
  ...
  "The expunge overdue tasks business logic's filtering methods" when {
    ...
    val taskCases = Table(
      ...
      ("expired inactive", Timestamp.zero, f.clock.now - f.strategy.timeUntilExpungeSeconds - 1.minute, Condition.UnreachableInactive, true     ),
      ("unreachable",      Timestamp.zero, f.clock.now - f.strategy.timeUntilInactiveSeconds,           Condition.Unreachable,         false    )
    )
    // format: ON
