...
class ExpungeOverdueLostTasksActorTest extends AkkaUnitTest with TableDrivenPropertyChecks {
  ...
  class Fixture {
    ...
    val handling = UnreachableInstanceHandling(5.minutes, 10.minutes)
  }
  ...
  "The expunge overdue tasks business logic's filtering methods" when {
    ...
    val taskCases = Table(
      ...
      ("expired inactive", Timestamp.zero, f.clock.now - f.handling.timeUntilExpunge - 1.minute, Condition.UnreachableInactive, true     ),
      ("unreachable",      Timestamp.zero, f.clock.now - f.handling.timeUntilInactive,           Condition.Unreachable,         false    )
    )
    // format: ON
    ...
    forAll(taskCases) { (name: String, startedAt: Timestamp, since: Timestamp, condition: Condition, expunge: Boolean) =>
      s"filtering $name task since $since" should {
        ...
        }).copy(unreachableInstanceHandling = f.handling)
